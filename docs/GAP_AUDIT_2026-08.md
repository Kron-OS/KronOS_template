...
| P1-5 (V8) | **V8 STATUS: RESOLVED, real bug found MORE nuanced than documented above.** Fixed with five `[FILTER] Name modify` blocks (real `Condition Key_Value_Equals level <value>` clauses, verified against the real 3.1.10 binary — zero config errors), each `Set`-ing a new `severity_num` field to the real RFC 5424 numeric severity (`debug=7, info=6, warning=4, error=3, critical=2` — the real, complete set of level strings this repo's logger emits, confirmed via `structlog.stdlib.map_method_name`'s own source plus a repo-wide grep of every `logger.*(` call site; `"warn"` is never actually emitted). The syslog `[OUTPUT]`'s `Syslog_Severity_Key` now reads `severity_num` instead of the raw `level` string. Real, previously-undocumented finding while verifying: the old bug did **not** uniformly fall back to one default severity as the Milestone S comment assumed — fluent-bit's own fallback keyword table happens to recognize `debug`/`info`/`warning` (RFC 5424's own severity names) and got those three right despite still logging `"invalid severity"`, while `error`/`critical` (which don't match RFC 5424's abbreviated keywords `err`/`crit`) silently landed as severity 6 (info) — meaning real ERROR/CRITICAL KronOS log lines were being under-reported as INFO to any downstream severity-based syslog routing, worse than "uniformly wrong." Proven against a real `fluent/fluent-bit:3.1.10` container and a real local UDP syslog receiver (`poc/fluent_bit_severity_and_nginx_logs/severity/`, real captured before/after: real `[warn] invalid severity` lines on every record before, zero after; real PRI-header decode confirming the exact correct severity landed for all 5 levels after). | `docker/fluent-bit/fluent-bit.conf`; `poc/fluent_bit_severity_and_nginx_logs/severity/` | Done |
...
| P1-6 (V8) | **V8 STATUS: RESOLVED, two real bugs found and fixed while verifying.** `docker/nginx/nginx.conf.template` now has a real `access_log`/`error_log` pair writing JSON lines (`log_format kronos_json escape=json ...`) to `/var/log/nginx/kronos-access.log` — matching `fluent-bit.conf`'s own pre-existing but previously-unfed `kronos.nginx` `Parser json` `[INPUT]` (path updated to match). `docker-compose.dev.yml`'s `nginx` service now mounts a real, canonically-declared `nginx_logs` volume at `/var/log/nginx` (mirroring `kronos_app_logs`); `docker-compose.fluent-bit.yml`'s own previously-plain/non-external `nginx_logs` declaration is now removed entirely (relies on the multi-file merge, exactly like `kronos_app_logs`/`falco_logs` already do). **Real bug #1** (found via direct image inspection before writing the fix): the official `nginx:alpine` base ships `access.log`/`error.log` as symlinks to `/dev/stdout`/`/dev/stderr`, and a freshly-mounted named volume copies the image directory's contents — including those symlinks — on first use; writing to the default filename would have silently followed the symlink back to stdout. Fixed by using a non-colliding filename (`kronos-access.log`). **Real bug #2** (found live, not anticipated): fluent-bit's own bundled `json` PARSER hardcodes `Time_Key time` + an Apache-log-style `Time_Format`; the first draft's JSON field `"time"` collided with it, producing a real captured `[warn] [parser:json] invalid time format ...` on every record. Fixed by renaming the field to `"timestamp"` (also now consistent with `kronos.backend`/`kronos.celery`'s own structlog field name). **Real bug #3, self-caught before landing** (Compose volume-naming, not nginx): an initial draft marked `nginx_logs` `external: true` in the fluent-bit overlay, which `docker compose config` proved resolves to the literal un-prefixed name `nginx_logs` — the exact bug class this same file's own header comment already documents as fixed for `kronos_app_logs`/`falco_logs`. Corrected to the no-redeclaration pattern before finishing; re-validated `nginx_logs` now resolves to `docker_nginx_logs`, matching the other two log volumes exactly. Proven end-to-end against a real `docker-nginx:latest` container + a real `fluent/fluent-bit:3.1.10` container + a real `curl` request (`poc/fluent_bit_severity_and_nginx_logs/nginx_access_log/`): the real access-log line for a real HTTP request landed in the real shared volume and flowed through fluent-bit's real `tail` → `json` PARSER → `record_modifier` FILTER → `stdout` OUTPUT with zero warnings. `docker compose config` re-validated clean on `docker-compose.dev.yml` alone and on the full `dev+wazuh+falco+fluent-bit` multi-file merge. | `docker/nginx/nginx.conf.template`; `docker/docker-compose.dev.yml`; `docker/fluent-bit/docker-compose.fluent-bit.yml`; `docker/fluent-bit/fluent-bit.conf`; `poc/fluent_bit_severity_and_nginx_logs/nginx_access_log/` | Done — follow-up noted: `charts/kronos/files/nginx.conf.template` (the Helm chart's own manually-duplicated copy of this file) was NOT updated with this fix — it has no `nginx_logs`-equivalent volume/fluent-bit sidecar wiring anywhere in `charts/kronos/templates/` today (Helm/K8s log-shipping is unstarted, consistent with P2-6), so there is no live consequence yet, but the two template copies are now further out of sync than before this pass; flagged for whoever picks up real K8s log-shipping wiring, not fixed here to stay in this item's own `docker/`-only scope. |
...
| P2-5 | ~~`kronos-attest case_report` still takes `events: list[dict]` from an offline audit-log export (`kronos_attest/report.py:71`) — confirmed by direct read, not fixed since `IMPROVEMENT_IDEAS.md` flagged it. For a platform whose flagship claim is court-admissible chain of custody, the report CLI not live-re-reading MinIO/Postgres/TSA at report time remains a real credibility gap.~~ **AA1 CLOSED (2026-08-18, commit `cbd4e79`):** `verify`/`day-report`/`case-report` now support `--database-url`/`--org-id` live-Postgres mode alongside the original `--audit-log` offline mode (mutually exclusive, validated). Reuses `PostgresAuditLogRepository.stream_by_org()` and `_to_export_dict()` verbatim (the exact same code the real `GET /api/audit/export` route uses) so live and offline modes are provably identical, not just similar — verified against the real shared dev Postgres: 23/23 real checks passed, byte-identical `merkle_root`/`chain_valid`/`event_count` between live and offline reports for the same real org (`poc/kronos_attest_live_mode/`). TSA verification was already fully real/offline (RFC 3161 tokens are self-verifying — no live TSA call is a meaningful concept); MinIO evidence-hash re-verification explicitly scoped out as a separate future item. See `docs/GAP_AUDIT_2026-08-18_MILESTONE_AA.md`. | `kronos_attest/report.py` | L |
