...
- [x] **A real, maintained `@playwright/test` E2E suite now exists** (`frontend/e2e/`, started 2026-08-28) — no longer just scattered one-off scripts. Four real specs passing together against the live dev stack: login (PKCE), evidence upload-to-COMPLETE (live SSE), evidence retry-to-COMPLETE (real forced OpenSearch outage + recovery — found the SSE-reconnect bug above), detection triage (NEW→INVESTIGATING, independently confirmed via a fresh API call). OOP page-object layer (`frontend/e2e/pages/`). Full status/delivery-order ledger: `docs/PLAYWRIGHT_E2E_TEST_PLAN.md` (still open: cross-tenant UI isolation, admin/org-settings, dashboards-embed/resilience/a11y specs, and the `docker-compose.test.yml` CI-capability gap needed before this suite can run unattended in CI).
