...
| LAN HTTPS access (nginx TLS termination + reverse proxies to Keycloak/MinIO/Dashboards) | Full `--no-cache` rebuild + fresh `up -d`, all 18 services healthy, zero errors/warnings across every container's logs; real login → real case → all 5 real evidence files uploaded through the HTTPS MinIO proxy with valid SigV4 signatures → autonomous pipeline → all 5 reached `COMPLETE` | [`poc/tls_lan_https/`](poc/tls_lan_https/README.md), [`docs/lan-dev-access.md`](docs/lan-dev-access.md) |
...
- [x] LAN HTTPS access — nginx terminates TLS (real step-ca-issued cert) for every browser-facing origin (SPA/API, Keycloak, MinIO presigned uploads, Dashboards iframe), fixing the secure-context requirement that blocked login from any non-`localhost` origin. Three real bugs found+fixed along the way (`step-ca`'s dead port mapping + always-green healthcheck; nginx's `$host` dropping the port and breaking MinIO's SigV4 validation; Keycloak's single-pinned-`KC_HOSTNAME` login form breaking the session-restart cookie across a `localhost`→LAN domain jump — the last one would have broken every `localhost` browser login, not just LAN access). Verified with the real `poc/full_ingestion_test/` flow end-to-end after a full `--no-cache` rebuild — [`poc/tls_lan_https/`](poc/tls_lan_https/README.md), [`docs/lan-dev-access.md`](docs/lan-dev-access.md)
