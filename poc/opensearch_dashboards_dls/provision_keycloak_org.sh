...
for USER_ID in $ORG_MEMBER_IDS; do
  ...
  # PoC step 3 (see poc/opensearch_jwt/option_a_flat_claim/keycloak_mapper_research.md):
  # flat org_id user attribute, the thing an oidc-usermodel-attribute-mapper
  # client scope turns into a flat top-level JWT claim -- OpenSearch DLS
  # templating (${attr.jwt.org_id}) cannot use the nested 'organization'
  # claim this script already produces via the org-membership call above.
  #
  # PUT .../users/{id} is NOT a partial update: confirmed empirically that a
  # body containing only {"attributes": {...}} silently clears firstName,
  # lastName, and email on the real user (Keycloak's Declarative User
  # Profile treats any profile-managed field missing from the request body
  # as "clear it", not "leave as-is") -- which then made every real
  # password-grant login fail with "Account is not fully set up". Fetching
  # the current representation and splicing the attributes object into it
  # (rather than sending attributes alone) avoids this; a fresh user has no
  # pre-existing "attributes" key at all (confirmed via GET), so this is a
  # correctness-preserving text-level splice, not a full JSON re-parse.
  CURRENT_USER=$(curl -s -H "Authorization: Bearer $TOKEN" "$KC_BASE/admin/realms/$KC_REALM/users/$USER_ID")
