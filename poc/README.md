...
| `tls_lan_https/` | LAN HTTPS access: real step-ca cert issuance (`tls-init`), nginx `:443` TLS termination (SPA/backend), and three reverse proxies — nginx→Keycloak `:8443`, nginx→MinIO `:9444`, nginx→Dashboards `:5602` — each checked alone then all together against the real dev stack, real root-CA-verified `curl` chains (not `-k`); re-verified after a full `--no-cache` rebuild by running the actual `poc/full_ingestion_test/` scripts (real login, real upload, real autonomous ingestion to `COMPLETE`) unmodified in intent against the rebuilt stack | 3 real bugs found + fixed: `step-ca`'s own `9443:9443` port mapping was dead (container actually listens on `:9000`; healthcheck's `\|\| exit 0` was silently masking it); nginx's `$host` variable drops the port, breaking MinIO's SigV4 presigned-URL signature validation through the proxy (403, confirmed with a real signed PUT) — fixed with `$http_host`; Keycloak's login form always POSTs to the single pinned `KC_HOSTNAME` origin regardless of where the flow started, so a `localhost`-originated login lost its session-restart cookie crossing to the LAN HTTPS origin (400 "Restart login cookie not found", a real bug that would have broken every `localhost` browser login) — fixed by making every client, `localhost` included, start at the one canonical origin |
